    ...
    return (
        <aside className={cn(
            ...
            <div className="px-4 flex items-center justify-between h-14 border-b border-slate-100 dark:border-white/5">
                {!isCollapsed && (
                    <span className="text-[9px] font-black tracking-[0.3em] uppercase animate-in fade-in duration-500 text-slate-400 dark:text-white/20">
                        Módulos
                    </span>
                )}
                <button
            ...
            <nav className="flex-1 px-3 py-4 space-y-6 overflow-y-auto custom-scrollbar pb-8">
                {sections.map((section, idx) => (
                    <div key={idx} className="space-y-0.5">
                        {!isCollapsed ? (
                            <h4 className="px-3 text-[9px] font-black tracking-[0.2em] mb-2 uppercase text-slate-400 dark:text-white/30">
                                {section.section}
                            </h4>
                        ) : (
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        activeView === item.id
                                            ...
                                            : "text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 border-l-2 border-transparent font-medium"
                                    )}
                                >
                                    <item.icon
                                        size={16}
